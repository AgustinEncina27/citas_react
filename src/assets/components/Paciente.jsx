...
function Paciente({paciente, setPaciente,eliminarPaciente}) {
  ...
  const handleEliminar=()=>{
    const respuesta= confirm("Deseas aliminar este pacietne?")
  ...
  return (
    <div className="mx-5 my-10 bg-white shadow-md px-5 py-10 rounded-xl">
      ...
      <div className="flex justify-between">
          <button 
            type="button"
            className="py-2 px-10 bg-indigo-600 hover:bg-indigo-700 text-white font-bold uppeercase rounded-lg"
            onClick={()=>setPaciente(paciente)}
          >Editar</button>
          <button 
            type="button"
            className="py-2 px-10 bg-red-600 hover:bg-red-700 text-white font-bold uppeercase rounded-lg"
